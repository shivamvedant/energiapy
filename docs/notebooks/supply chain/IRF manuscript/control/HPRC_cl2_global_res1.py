...
def supply_chain_controller(time_ind: List, norm_varying_dict: Dict, start_at: Union[int, Tuple] = 0,
    ...
    for t in range(start_idx, end_idx + 1, 1):
        ...
        if t == 168:
            model.write('model_cl2_global_res1.lp', io_options={'symbolic_solver_labels': True})
            # model.write('model_cl2_local_res1.mps')
...
if __name__ == '__main__':
    ...
    with open(f'../cl2/output_256_00_cl2_global.pkl', 'rb') as file:
    ...
    scen_final, model_final = build_schedule_model(start_time=scale_iter_list[0], end_time=scale_iter_list[-1],
                                                   scen_df=pandas.DataFrame(data={('loc1', 'com1_pur'): disruption_dict[('loc1', 'com1_pur')][
                                                       list(disruption_dict[('loc1', 'com1_pur')].keys())[-1]]}))
    ...
    with open(f'ControlResults_cl2_global_res1.pkl', 'wb') as file:
