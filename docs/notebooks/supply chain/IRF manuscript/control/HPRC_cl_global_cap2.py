...
def supply_chain_controller(time_ind: List, norm_varying_dict: Dict, start_at: Union[int, Tuple] = 0,
    ...
    for t in range(start_idx, end_idx + 1, 1):
        print(f"Starting time: {t} \t time_index: {time_ind[t]}")
        t0 = max(0, t - lookback)
        ...
        # if 84 <= t <= 95:
        #     print(scen_df[[('loc2', 'com1_process')]].dropna())

        # print(type(scen_df[[('loc2', 'com1_process')]].dropna()), len(scen_df[[('loc2', 'com1_process')]].dropna()))
        ...
        if t == 84:
            model.write('model_cl_global_cap2.lp', io_options={'symbolic_solver_labels': True})
            # model.write('model_cl_global_cap2.mps')
...
if __name__ == '__main__':
    ...
    with open(f'../cl/output_128_00_cl_global.pkl', 'rb') as file:
        load_output_dict = pickle.load(file)

    design_scen = list(load_output_dict.keys())[0]

    disruption_time_idx = 14  # Disruption starts here
    early_warning = 1  # Information about the disruption is relayed to the model
    capacity_factor_dict = {
        k: [1] * (disruption_time_idx - 1) + [int(i < disruption_time_idx - early_warning - 1)] * (
                schedule_exec_scenarios - disruption_time_idx + 1) for i in range(schedule_exec_scenarios)
    ...
    close_loop_results, end_result, end_model, end_scenario = supply_chain_controller(time_ind=scale_iter_list,
                                                                                      norm_varying_dict=disruption_dict,
                                                                                      p_horizon=30,
                                                                                      design_model=load_output_dict[
                                                                                          design_scen])

    scen_final, model_final = build_schedule_model(start_time=scale_iter_list[0], end_time=scale_iter_list[-1],
                                                   scen_df=pandas.DataFrame(data={('loc2', 'com1_process'):
                                                                                      disruption_dict[
                                                                                          ('loc2', 'com1_process')][
                                                                                          list(disruption_dict[('loc2',
                                                                                                                'com1_process')].keys())[
                                                                                              -1]]}))
    ...
    with open(f'ControlResults_cl_global_cap2.pkl', 'wb') as file:
